...
# Import 2022 Metropolitan DDAs, from https://www.huduser.gov/portal/datasets/qct.html
sadda <- read_xlsx('../data/2022-DDAs-Data-Used-to-Designate.xlsx', sheet=1)

# Import zcta to FIPS crosswalk, from https://www.census.gov/geographies/reference-files/time-series/geo/relationship-files.2010.html 
...
sadda_model <-lm_robust(sadda~qct+yr_alloc+non_prof+trgt_pop+trgt_fam+trgt_eld+trgt_hml+nine_prcnt , data = lihtc, fixed_effects = proj_st)
...
qct_model <-lm_robust(qct~yr_alloc+non_prof+trgt_pop+trgt_fam+trgt_eld+trgt_hml+nine_prcnt , data = lihtc, fixed_effects = proj_st)
...
op_nbh_model <-lm_robust(op_nbh~yr_alloc+non_prof+trgt_pop+trgt_fam+trgt_eld+trgt_hml+nine_prcnt , data = lihtc, fixed_effects = proj_st)

modelsummary(op_nbh_model, stars = TRUE, output = 'latex')
